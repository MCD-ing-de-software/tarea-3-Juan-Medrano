...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        w = StatisticsUtils()
        x = w.moving_average(arr = v, window = 2)
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        """
        ## a: Crear una secuencia bidimensional (ej: [[1, 2], [3, 4]])
        v = [[1, 2], [3, 4]]
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        """
        ## a: Crear una lista de números (ej: [10, 20, 30, 40])
        v = [10, 20, 30, 40]
        ## b: Llamar a zscore para obtener los z-scores (resultado es un array de NumPy)
        x = StatisticsUtils()
        w = x.zscore(v)
        ## c: Verificar que la media del resultado es aproximadamente 0 (usar self.assertAlmostEqual para un solo valor numérico - unittest es suficiente)
        self.assertAlmostEqual(np.mean(w), 0)
        ## d: Verificar que la desviación estándar del resultado es aproximadamente 1 (usar self.assertAlmostEqual para un solo valor numérico - unittest es suficiente)
        self.assertAlmostEqual(np.std(w), 1)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        """
        ## a: Crear una lista con todos los valores iguales (ej: [5, 5, 5])
        v = [5, 5, 5]
        ## b: Llamar a zscore con esa secuencia y verificar que se lanza un ValueError indicando que la desviación estándar es cero (usar self.assertRaises)
        x = StatisticsUtils()
        with self.assertRaises(ValueError):
            x.zscore(v)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        """
        ## a: Crear una lista de números (ej: [2, 4, 6])
        v = [2, 4, 6]
        ## b: Llamar a min_max_scale para obtener los valores escalados (resultado es un array de NumPy)
        x = StatisticsUtils()
        w = x.min_max_scale(v)
        ## c: Verificar que el valor mínimo del resultado es 0.0 (usar self.assertAlmostEqual para un solo valor numérico - unittest es suficiente)
        self.assertAlmostEqual(np.min(w), 0)
        ## d: Verificar que el valor máximo del resultado es 1.0 (usar self.assertAlmostEqual para un solo valor numérico - unittest es suficiente)
        self.assertAlmostEqual(np.max(w), 1)
        ## e: Verificar que los valores transformados son correctos (ej: [0.0, 0.5, 1.0] para [2, 4, 6]) (usar numpy.testing.assert_allclose() para comparar el array completo - esto es necesario para comparar arrays de NumPy con tolerancia para errores de punto flotante)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        ## a: Crear una lista con todos los valores iguales (ej: [3, 3, 3])
        v = [3, 3, 3]
        ## b: Llamar a min_max_scale con esa secuencia y verificar que se lanza un ValueError indicando que todos los valores son iguales (usar self.assertRaises)
        x = StatisticsUtils()
        with self.assertRaises(ValueError):
            x.min_max_scale(v)
