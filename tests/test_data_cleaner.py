...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        """
        ## a: Crear un DataFrame con valores faltantes usando make_sample_df()
        df = make_sample_df()
        ## b: Llamar a drop_invalid_rows con las columnas "name" y "age"
        x = DataCleaner()
        w = x.drop_invalid_rows(df, ["name", "age"])
        ## c: Verificar que el DataFrame resultante no tiene valores faltantes en esas columnas (usar self.assertEqual para comparar .isna().sum() con 0 - comparación simple de enteros, unittest es suficiente)
        n = w[["name", "age"]].isna().sum().sum()
        self.assertEqual(n, 0)
        ## d: Verificar que el DataFrame resultante tiene menos filas que el original (usar self.assertLess con len() - comparación simple de enteros, unittest es suficiente)
        self.assertLess(len(w), len(df))
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        df = make_sample_df()
        ## b: Llamar a drop_invalid_rows con una columna que no existe (ej: "does_not_exist")
        z  = DataCleaner()
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        ## a: Crear un DataFrame usando make_sample_df()
        df = make_sample_df()
        ## b: Llamar a trim_strings con una columna numérica (ej: "age")
        ## c: Verificar que se lanza un TypeError (usar self.assertRaises)
        x = DataCleaner()
        with self.assertRaises(TypeError):
            x.trim_strings(df, ["age"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        ## a: Crear un DataFrame con valores extremos usando make_sample_df() (contiene edad=120)
        df = make_sample_df()
        ## b: Llamar a remove_outliers_iqr con la columna "age" y factor=1.5
        x = DataCleaner()
        w = x.remove_outliers_iqr(df, col = "age", factor = 1.5)
        ## c: Verificar que el valor extremo (120) fue eliminado del resultado (usar self.assertNotIn para verificar que 120 no está en los valores de la columna)
        self.assertNotIn(120, w.age)
